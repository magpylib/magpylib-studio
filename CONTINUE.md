...
- `vscode-extension/` — the TS shell, **compiles clean, smoke-tested**:
  ...
  - Layout: tree click → host `selectedObjectId` → inspector loads it; the
    ...
    node per studio id) and can be worked in: pick, multi-select, drag,
    resize, aim, hide, play the paths. A **Chart** button swaps in the Plotly
    figure, read only, with its own animation toggle (template follows the VS
    Code theme). `broadcastMutation()` (debounced 150 ms) refreshes plot +
    tree + inspector + virtual docs after every edit from any surface
    (inspector widgets, LM tools, tree commands); a drag previews without it,
    paced one request in flight.
